      ...
      <aside className={`fixed inset-y-0 left-0 z-30 flex w-[252px] flex-col bg-[#173937] px-5 py-6 text-[#f9f5e9] transition-transform duration-300 lg:translate-x-0 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex items-center justify-between px-2">
          <Link href="/" className="flex items-center gap-3" onClick={() => setMobileOpen(false)} data-testid="link-brand">
            <div className="flex h-10 w-10 items-center justify-center rounded-[13px] bg-[#f5b84f] text-[#173937]"><HeartHandshake className="h-5 w-5" strokeWidth={2.5} /></div>
            <div><div className="serif text-[22px] leading-none tracking-[-.02em]">CareLedger</div><div className="mt-1 text-[9px] font-semibold uppercase tracking-[.2em] text-[#adc4b5]">Steady together</div></div>
          </Link>
        ...
        <div className="mt-12 px-3 text-[10px] font-bold uppercase tracking-[.22em] text-[#86a99a]">Your workspace</div>
        <nav className="mt-3 space-y-1" aria-label="Main navigation">
          {navItems.map(({ href, label, icon: Icon }) => {
            const active = location === href;
            return <Link key={href} href={href} onClick={() => setMobileOpen(false)} className={`group flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-medium ${active ? 'bg-[#f5b84f] text-[#173937]' : 'text-[#d4e2d8] hover:bg-[#28504c]'}`} data-testid={`link-nav-${label.toLowerCase()}`}>
        ...
        <div className="mt-auto">
          ...
          <div className="mt-5 rounded-2xl border border-[#32615a] bg-[#214744] p-4">
            <div className="mb-3 flex items-center justify-between"><Sparkles className="h-4 w-4 text-[#f5b84f]" /><span className="text-[10px] font-bold uppercase tracking-[.15em] text-[#86a99a]">Care note</span></div>
          ...
          <div className="mt-5 flex items-center gap-3 border-t border-[#32615a] pt-5">
            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-[#f0c9a6] text-xs font-bold text-[#173937]">MC</div>
...
function DocumentsPage() {
  ...
  return <div><PageIntro eyebrow="The paper trail" title="Everything important, together." copy="A calm home for the documents that make Maya's plan understandable, portable, and protected." action={<button className="flex w-fit items-center gap-2 rounded-full bg-[#ebe6da] px-5 py-3 text-sm font-semibold text-[#173937] hover:bg-[#e2dccf]" onClick={() => window.alert('Document uploads will be available in the next CareLedger release.')} data-testid="button-add-document"><Plus className="h-4 w-4" /> Add document</button>} /><div className="mb-6 flex items-center justify-between"><div className="flex items-center gap-2 text-sm text-[#68766e]"><FileCheck2 className="h-4 w-4 text-[#68a27b]" /> {documents.filter((document) => document.status === 'ready').length} documents ready</div><button className="flex items-center gap-2 rounded-lg px-2 py-2 text-sm font-semibold text-[#68766e] hover:bg-[#ebe6da]" data-testid="button-document-sort">Recently updated <ChevronDown className="h-4 w-4" /></button></div>{documents.length ? <div className="grid gap-4 md:grid-cols-2">{documents.map((document) => <DocumentCard key={document.id} document={document} />)}</div> : <EmptyState icon={FileText} title="Your shelf is ready when you are" copy="Keep letters, statements, and plan documents together so they are there when someone asks." action={<button className="mt-5 rounded-full bg-[#f5b84f] px-5 py-2.5 text-sm font-bold text-[#173937]" onClick={() => window.alert('Document uploads will be available in the next CareLedger release.')} data-testid="button-empty-add-document">Add first document</button>} />}</div>;
}
